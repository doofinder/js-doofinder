...
function buildQuery(query?: string, params?: object) {
  ...
  return (cfg.getClient() as any)._buildSearchQueryString(query, params);
}
...
describe("Client", () => {
  ...
  context("Instantiation", () => {
    ...
    context("with API Key and zone", () => {
      it("should use API key's zone", (done) => {
        ...
        (client as any).requestOptions.host.should.equal("eu1-search.doofinder.com");
        done();
      });
    });
  ...
  context("Request", () => {
    it("handles request errors gracefully", async () => {
      ...
      response.data.error.should.equal("search engine not found");
  ...
  context("Options", () => {
    it("Call with no arguments is done to the correct URL", async () => {
      ...
      expect(fetchMock.called(`${cfg.address}/${cfg.version}/options/${cfg.hashid}`)).to.be.true;
      response.data.should.be.empty;
    });

    it("Call with argument generates the correct url params", async () => {
      const response = await cfg.getClient().options("example.com");
      expect(fetchMock.called(`${cfg.address}/${cfg.version}/options/${cfg.hashid}?example.com`)).to.be.true;
